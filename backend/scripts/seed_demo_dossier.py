...
async def seed_company(session, id: uuid.UUID, name: str, website: str, job_url: str, status: str, fit_score: float) -> models.Company:
    ...
    async with AsyncSessionLocal() as session:
        ...
        evidence_events_aetheria = [
            (
                "Tavily",
                "source",
                {"urls": ["https://aetheria-ai.com", "https://aetheria-ai.com/about"], "snippet": "Aetheria AI is building next-generation agentic developer platforms. Based in Munich, Series A funding."}
            ),
            (
                "Pioneer",
                "entity_extraction",
                {"entities": {"tech_stack": ["Python", "FastAPI", "PostgreSQL", "Next.js", "React", "Gemini"], "headcount": "10-50"}, "labels": ["Tech Stack", "Growth", "Culture"]}
            ),
            (
                "Gemini",
                "reasoning",
                {"reasoning": "Strong match because of direct alignment between FastAPI backend agent experience and Aetheria's agentic framework. Highly relevant Series A growth stage.", "score": 0.92, "labels": ["Leadership", "Funding"]}
            ),
            (
                "Telegram",
                "approval_state",
                {"approved": True, "comment": "Aetheria AI matches your FastAPI agent profile perfectly! Approved by user."}
            ),
            (
                "fal",
                "visual_artifact",
                {"image_url": "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=800&auto=format&fit=crop&q=60", "prompt": "Futuristic AI agents collaborating to build software, modern tech aesthetic, synthwave style"}
            )
        ]
        ...
        print(f"- Company Aetheria AI (approved, fit_score: 0.92)")
        print(f"  └─ Research: Series A, VP of Engineering, tech_stack: {['Python', 'FastAPI', 'PostgreSQL', 'Next.js', 'React', 'Gemini']}")
        print(f"  └─ EvidenceEvents: Tavily, Pioneer, Gemini, Telegram (approved), fal (success)")
        print(f"  └─ Message: LinkedIn message (approved)")
        print(f"- Company Nebula Robotics (researched, fit_score: 0.65)")
        print(f"  └─ Research: Seed, Co-Founder & CTO, tech_stack: {['Python', 'C++', 'ROS2', 'PyTorch']}")
        print(f"  └─ EvidenceEvent: fal (error: timeout)")
